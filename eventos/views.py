...
def inscrever_evento(request, id):
    ...
    if request.method == "GET":
        ...
        if request.user in evento.participantes.all():
            messages.add_message(request, constants.INFO, 'Você já está inscrito neste evento')
...
def gerar_certificado(request, id):
    ...
    qtd_certificados = evento.participantes.all().count() - Certificado.objects.filter(evento=evento).count()
    if qtd_certificados < 1:
        messages.add_message(request, constants.INFO, 'Não há certificados para gerar')
    ...
    for participante in evento.participantes.all():
        ...
        draw.text((220,631), f"{participante.username}", font=fonte_nome, fill=(0,0,0))
        draw.text((761,767), f"{evento.nome}", font=fonte_info, fill=(0,0,0))
        draw.text((816,836), f"{evento.carga_horaria} horas", font=fonte_info, fill=(0,0,0))
